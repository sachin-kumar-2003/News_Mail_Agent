...
function Main() {
    const serverUrl = "http://127.0.1:8000"
    const [usermail, setMail] = useState("")
    const [userQuery, setQuery] = useState("")

    const  handleSubmit = async() =>{

        const response = await axios.post(`${serverUrl}/news`,{
            usermail: usermail,
            userQuery:userQuery
        })
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
        ...
        <div className="w-full md:w-auto space-y-6">
          ...
          <div className="space-y-3 p-4 bg-blue-50 rounded-xl border border-blue-100">
            ...
            <button className="w-full flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">
              <span>Send mail</span>
              <IoSend />
            </button>
          </div>
